...
# Rule: Pos 0-3 can hu, except true winner at the end of the round.
    ...
    if winner_id != -1 and min(init_dist) != 0:
        ...
        for id in range(winner_id + 1, 4):
            ...
            if bz_log_init_pointer <= 33:
                holder_for_completing_round[id] = init_tilewall_list[id][
                    bz_log_init_pointer
                ]

        # consider new potential winner, except true one
        if len(holder_for_completing_round.keys()) > 0:
            for id in range(4):
                if id != winner_id:
                    # for each player
                    final_hand_tmp = handWall[-1][id]
                    final_wall_tmp = tileWall[-1][id]
                    final_pack_tmp = pack[-1][id]
                    for key_id in holder_for_completing_round:
                        tile_considered = holder_for_completing_round[key_id]
                        ...
                        if result >= 8 and id not in completing_round_winner:
                            completing_round_winner.append(id)
